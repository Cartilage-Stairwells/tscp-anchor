...
impl OWSLStatus {
    ...
    /// ARCHER Finding 6 fix: verify content_hash matches recomputed hash.
    /// Replaces hardcoded `checksum_valid = True` from the daemon.
    ...
    pub fn read_status(&mut self) -> Result<OWSLStatus, String> {
        ...
        if !status.is_fresh(self.max_age_seconds) {
            return Err(format!(
                ...
                self.max_age_seconds, status.timestamp
            ));
        }
    ...
    pub fn permits_verification(&mut self) -> bool {
        match self.read_status() {
            Ok(status) => {
                ...
                if !permits {
                    eprintln!(
                        "[OWSL-Bridge] Verification BLOCKED: status={}, action={}, remaining={}",
...
mod tests {
    ...
    fn create_test_status_json(status: &str, action: &str) -> String {
        let ts = SystemTime::now()
            ...
            .unwrap()
            .as_secs_f64();
        // Compute content hash matching the daemon's algorithm
        let hash_input = format!("{}|{}|{}|42|32|96||10|0|1000", ts, status, action);
